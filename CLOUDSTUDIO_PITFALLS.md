...
- Go 服务用 **`GOMEMLIMIT`** 钉住堆上限，不要指望它自觉。**已落地**：new-api 的启动器（`/workspace/.keepalive/start.d/10-newapi.sh`）加了 `GOMEMLIMIT=192MiB`——它是软上限，超了只会 GC 更勤、不会被杀，重启后实测 RSS 97 MB → 68 MB，`/proc/<pid>/environ` 可验
