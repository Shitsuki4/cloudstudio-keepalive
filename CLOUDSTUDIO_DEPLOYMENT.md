...
> **实测更正**：所谓"每天凌晨 2:00-4:00 重启"对 **RUNNING** 工作区是 **no-op**。实测曾观测到某工作区跨越 **9 个** 04:00 窗口零重建（启动日志无新记录、PID1 未变）。只有 **STOP → RUN** 才会真正重建容器。详见 [部署踩坑实录](./CLOUDSTUDIO_PITFALLS.md)。

- **触发条件**：只有 STOP → RUN 会重建容器；RUNNING 状态下定时触发的 RunWorkspace 不重建、也不跑启动钩子
- **重启后状态**：
  - ✅ 持久化目录（`~/workspace`）数据保留
  - ❌ 系统进程全部停止
  - ❌ 环境变量重置
  - ❌ 临时文件清空（`/tmp`）
- **不要把"每天会自动恢复一次"当作容错假设**，服务必须自己能长期稳定运行

### ⚠️ 自启动机制：优先用 supervisor 的 start.d

工作区 PID1 是 **supervisord**（不是 systemd），`.zshrc` 只在交互式登录时执行、容器冷启动不跑，`systemctl` 不可用。

**可靠路径**：`/workspace/.keepalive/boot.sh` 由 supervisord include 调用 → 顺序执行 `start.d/*.sh`。详见 [部署踩坑实录](./CLOUDSTUDIO_PITFALLS.md)。

### 🎯 核心策略

1. **持久化一切**：配置、数据、凭据都放在 `~/workspace` 下
2. **自启动必配**：在 `/workspace/.keepalive/start.d/` 下放幂等脚本（`.zshrc` / PM2 对冷启动无效）
...
> **推荐做法**：把启动脚本放到 `/workspace/.keepalive/start.d/`（由 supervisord 调用的 `boot.sh` 顺序执行），而不是写进 `.zshrc`。
> 容器冷启动**不会**执行 `.zshrc`，只靠它自启在真实重建后会失效。详见 [部署踩坑实录](./CLOUDSTUDIO_PITFALLS.md)。
> 下面 3.1 的方式仅在"你确实需要交互式登录时顺带拉起"的场景下使用。
...
> **注意**：PM2 与本工作区的原生启动链**不兼容且不必要**——`pm2 resurrect` 依赖 shell 启动，而容器冷启动不执行 `.zshrc`；PM2 还需额外安装（重建后丢失）。**推荐直接用 `start.d`**（见 [部署踩坑实录](./CLOUDSTUDIO_PITFALLS.md)）。以下内容仅在你有特殊需求时参考。
...
2. ✅ 用 `start.d/*.sh` 自启动（不是 PM2、不是 `.zshrc`）
3. ✅ 启动脚本幂等：服务已在跑就直接 `exit 0`
4. ✅ 凭据文件独立管理
5. ✅ 使用绝对路径
6. ✅ 定期备份重要数据
...
- [ ] 目录结构创建完成
- [ ] 启动脚本可执行且幂等（已在跑则 `exit 0`）
- [ ] 脚本名不含服务二进制的名字（避免被按名杀进程误杀）
- [ ] 已放入 `/workspace/.keepalive/start.d/`（不依赖 `.zshrc` / PM2）
- [ ] 凭据文件已创建
- [ ] .gitignore 已配置
- [ ] 手动启动测试成功
- [ ] **杀掉服务后 `boot.sh manual` 能自动拉起**
- [ ] **服务在跑时 `boot.sh manual` 能正确跳过（幂等）**
